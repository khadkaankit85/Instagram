...
const VerifyEmail = () => {
  ...
  const handleClick = (e: React.KeyboardEvent<HTMLInputElement>) => {
    let key = e.key;
    let previousElement = e.currentTarget.previousElementSibling as HTMLElement;
    let nextElement = e.currentTarget.nextElementSibling as HTMLElement;
    if (key == "Backspace") {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    let field = e.target.name as "val1" | "val2" | "val3" | "val4" | "val5";
  ...
  return (
    <div className="w-[100vw] h-[calc(100vh-6rem)]">
      ...
      <main className="w-[89vw] mx-auto h-[90vh] border border-solid md:w-[500px] lg:-[40vw] mt-6 flex justify-between flex-col">
        <div>
          <h1 className="text-white text-3xl mt-9 ml-5">Enter Code</h1>
          <h3 className="text-white mt-3 ml-5">
            We sent an email with <span className="text-yellow-500 ">OTP</span>{" "}
            to your account
          </h3>
          <form action="" id="otpForm">
            <input
              name="val1"
              type="text"
              className=" w-12 h-14 ml-5 mt-5 text-5xl text-center appearance-none"
              onChange={(e) => {
                handleChange(e);
              }}
            ...
            <input
              name="val2"
              type="text"
              className="w-12 h-14 ml-5 mt-5 text-5xl text-center"
              onKeyUp={(e) => handleClick(e)}
              onChange={(e) => {
                handleChange(e);
              }}
        ...
        <button
          className="w-[80%] h-6 bg-white mx-auto p-7 flex justify-center items-center mb-11 text-2xl text-center text-white-600 active:bg-[#9b9696]"
          onClick={async () => {
            ...
            if (otpToSubmit.length != 5) {
              ...
              try {
                let response = await fetch(
                  "http://localhost:3500/otp/verifyOtp",
                  {
                    method: "POST",
                    headers: {
                ...
                if (response.status == 200) {
                  toast.info(
                    "Account created Succcessfully, redirecting to login page in 5 seconds",
                    {
                      position: "top-right",
                      autoClose: 5000,
                      hideProgressBar: false,
        ...
        >
          <span className="text-black text-center">Verify</span>
        </button>
      </main>
